...
if (length(args)< 1) {
  stop("1 arguments must be supplied: graph file.n", call.=FALSE)
} 

workingDir <- "."
dataDir <- "Results/expanded_modules_networks"
setwd(workingDir)
...
network = read_graph(file.path(dataDir, networkfile), format = "gml") 
...
mc <- newLinkCommsAt(lc, cutat = 0.995)
...
save(lc, file = file.path('Results/linkcomm', paste(subnet,"_lc.RData", sep = "")))
save(mc, file = file.path('Results/linkcomm', paste(subnet,"_mc.RData", sep = "")))
...
write_graph(network.mc, file = file.path('Results/linkcomm', paste(subnet,"_mc.gml", sep = "")), format = "gml")
        
#### Enrichment of the link communities. Note working with the merged communities

source("RCode/enrichment.R")

mc_enrich <- function(linkcomms=mc, community){
